<svelte:head>
  <title>Why API Builder? - API Builder</title>
</svelte:head>
...
<div>
  ...
  <p class="text-ab-dark-blue mb-8 text-lg leading-relaxed">
    ...
    interfaces, and the compiler verifies everything matches.
  </p>
  ...
  <div class="mb-10">
    ...
    <ul class="space-y-5 text-ab-dark-blue ml-10">
      ...
      <li>
        <span class="font-semibold">The compiler becomes the test oracle</span> — If your implementation compiles against the generated interfaces,
        it matches the spec. Compiler errors tell the agent exactly what's wrong — missing methods, wrong types, incorrect signatures.
      </li>
      <li>
        <span class="font-semibold">Mock clients for testing</span> — Generated mock clients share the same interface definition. Agents write
        meaningful tests without external dependencies and override only what's needed for each scenario.
      </li>
    </ul>
  </div>
  ...
  <div class="mb-10">
    ...
    <ul class="space-y-5 text-ab-dark-blue ml-10">
      ...
      <li>
        <span class="font-semibold">Spec as design input</span> — Agents use the explicit data shapes to plan UX, database schemas, and data flows.
        The spec becomes input to both API implementation and frontend design.
      </li>
      <li>
        <span class="font-semibold">First class union types</span> — A type constrained to one of a known set. Agents know every valid state a
        value can be in — no open-ended polymorphism to handle.
      </li>
    </ul>
  </div>
  ...
  <div class="mb-10">
    ...
    <ul class="space-y-5 text-ab-dark-blue ml-10">
      ...
      <li>
        <span class="font-semibold">Fully expanded output</span> — The service specification is fully expanded and complete. An agent can read
        it and understand the entire API surface in one pass — no external references to follow.
      </li>
    </ul>
  </div>
  ...
  <div class="mt-12 mb-8 text-center">
    <p class="text-ab-dark-blue mb-4">Ready to try it?</p>
