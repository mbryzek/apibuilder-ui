...
<div class="mb-6 flex flex-wrap items-center gap-3">
  ...
  <div class="flex items-center gap-2">
    {#if data.session}
      <PendingForm action="{versionBase}?/{data.isWatching ? 'unwatch' : 'watch'}">
        {#snippet children(pending)}
          <button
            type="submit"
            disabled={pending || !!data.watchLoadError}
...
{#if showDeleteConfirm}
  <Banner>
    ...
    <div class="flex gap-3">
      <PendingForm action="{versionBase}?/deleteVersion">
        {#snippet children(pending)}
          <button type="submit" class="btn-danger" disabled={pending}>
            {pending ? 'Deleting...' : 'Yes, Delete Version'}
          </button>
        {/snippet}
      </PendingForm>
