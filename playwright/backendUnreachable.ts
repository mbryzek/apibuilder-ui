 ...
 * backend stopped fails; `csp.spec.ts`, which asserts headers the frontend serves on its own, keeps passing whatever happened to the backend.
 *
 * WHY THIS IS AT THE FETCH SEAM rather than at the call sites: every fixture reaches the backend
 * through a generated client (`test-helpers.ts` builds the one platform client), and a client's `fetch` option is the one place all of
...
export function backendUnreachable(error: unknown): Error | undefined {
  ...
  return new Error(
    `The e2e backend at ${config.API_BASE_URL} is not answering (${transportCause(error)}), so nothing was ` +
